...
export class AppComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    private assignment1() {
        let result = null;
        const condition = this.filterStatus().every((object) => object.invoice.isValid);
        result = condition ? 'success' : this.extractInvalidInvoices();
        console.log(result);

    }

    private extractInvalidInvoices(){
     ...
     */
    private assignemnt2() {
        const newRequest = { id: 10, description: 'new description', title: 'new title' } as Request;
        const modifiedData = [newRequest, ...data];
        const foundRequest = modifiedData.find(request => request.id === 10) || newRequest;
        const { id, title } = foundRequest;
